...
Packages:
```{r}
# If you wish to use `renv` package to handle R packages and dependency to keep your work reproducible (at least slightly, the next level would be to use a Rocker image to dockerize the code using containers) in the root of the project folder where "example-proj-name.Rproj" is located, open it and, do `renv::init()`. You need to have `renv` package installed, i.e., install.packages(c("renv")).

# You need to run the following line to install required packages.
# install.packages(c("tidyverse", "here", "readxl", "janitor"))
...
Population data:
```{r}
d_male <- read_xlsx(here("data/WPP2024_POP_F01_2_POPULATION_SINGLE_AGE_MALE.xlsx"), skip = 16, col_types = "text") |>
  mutate(Year = as.numeric(Year))
d_male$sex <- "Male"
d_male <- d_male |> drop_na(Year)
d_female <- read_xlsx(here("data/WPP2024_POP_F01_3_POPULATION_SINGLE_AGE_FEMALE.xlsx"), skip = 16, col_types = "text") |>
  mutate(Year = as.numeric(Year))
d_female$sex <- "Female"
...
```{r}
d_male <- read_xlsx(here("data/WPP2024_MORT_F01_2_DEATHS_SINGLE_AGE_MALE.xlsx"), skip = 16, col_types = "text") |>
  mutate(Year = as.numeric(Year))
d_male$sex <- "Male"
d_male <- d_male |> drop_na(Year)
d_female <- read_xlsx(here("data/WPP2024_MORT_F01_3_DEATHS_SINGLE_AGE_FEMALE.xlsx"), skip = 16, col_types = "text") |>
  mutate(Year = as.numeric(Year))
d_female$sex <- "Female"
d_female <- d_female |> drop_na(Year)
...
total_pops |> 
  ...
  ggplot(aes(x=year, y=CDR, color = region)) + 
  geom_line()
