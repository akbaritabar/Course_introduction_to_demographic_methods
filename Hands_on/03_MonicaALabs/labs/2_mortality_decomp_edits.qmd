...
dlt_alternative$lx <- NA
dlt_alternative$lx[1] <- 1

for(i in 2:nrow(dlt_alternative)){
  dlt_alternative$lx[i] <- dlt_alternative$lx[i-1]*dlt_alternative$px[i-1]
}

dlt_alternative$dx <- NA
for(i in 1:nrow(dlt_alternative)){
  dlt_alternative$dx[i] <- dlt_alternative$lx[i] - dlt_alternative$lx[i+1]
}

dlt_alternative$Lx <- NA

for(i in 1:nrow(dlt_alternative)){
  dlt_alternative$Lx[i] <- dlt_alternative$n[i]*dlt_alternative$lx[i+1] +dlt_alternative$ax[i]*dlt_alternative$dx[i]
}
...
```{r}
dl2 <- d |> 
  filter(year==2012|year==2016, cause=="Total, all causes of death  [A00-Y89]") %>% 
  mutate(age = start_age, Mx = rate/100000) %>% 
  select(year, age, Mx)
...
# let's calculate lifespan disparity for years 2012 and 2016

d_lt2 |> 
  group_by(year) |> 
  mutate(prod = dx*ex) |> 
  summarize(ld = sum(prod))

d_lt2 |> 
  ggplot(aes(age, dx, color = year, group = year))+
  geom_line()
```
...
lt_cd <- dls |> 
     mutate(n = case_when(
      ...
      Tx = rev(cumsum(rev(Lx))), # sum of Lx above age x
      ex = Tx / lx,
     ## CAUSE DELETED COLUMNS 
     px_minusi = 1 - qx_minusi, # probability of survival
      lx_minusi = lag(cumprod(px_minusi), default = 1), # l(x+n) = lx*px
      dx_minusi = lx_minusi - lead(lx_minusi, default = 0), # ndx = lx - l(x+n)
      Lx_minusi = n * lead(lx_minusi, default = 0) + (ax* dx_minusi), # nLx = n*l(x+n) + nax*ndx
      Tx_minusi = rev(cumsum(rev(Lx_minusi))), # sum of Lx above age x
...
DO NOT DO!!!! MONICA GOT TOO EXCITED!!!
...
```{r}
d_male <- read_xlsx(
  here("data/WPP2024_POP_F01_2_POPULATION_SINGLE_AGE_MALE.xlsx"),
  skip = 16,
  col_types = "text"
) |>
...
d_female <- read_xlsx(
  here("data/WPP2024_POP_F01_3_POPULATION_SINGLE_AGE_FEMALE.xlsx"),
  skip = 16,
  col_types = "text"
) |>
  mutate(Year = as.numeric(Year))
...
d_male <- read_xlsx(
  here("data/WPP2024_MORT_F01_2_DEATHS_SINGLE_AGE_MALE.xlsx"),
  skip = 16
)
d_male$sex <- "Male"
d_male <- d_male |> drop_na(Year)
d_female <- read_xlsx(
  here("data/WPP2024_MORT_F01_3_DEATHS_SINGLE_AGE_FEMALE.xlsx"),
  skip = 16
)
d_female$sex <- "Female"
d_female <- d_female |> drop_na(Year)
