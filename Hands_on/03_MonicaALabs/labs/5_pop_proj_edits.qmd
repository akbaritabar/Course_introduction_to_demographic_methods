...
```{r}
# Read and select variables for age-specific fertility rates
df <- read_csv(here("data/WPP2024_Fertility_by_Age5.csv"))
df <- df |> 
  select(Location, Time, ASFR, AgeGrp)

# Read and select female life table survivorship columns
dl <- read_csv(here("data/WPP2024_Life_Table_Abridged_Medium_1950-2023.csv"))
dl <- dl |> 
  filter(Variant=="Medium", Sex=="Female")|> 
  select(Location, Time, AgeGrp,Lx)

# Read female population counts, reshape to long format, and calculate single-year ages
d_female <- read_csv(here("data/wpp_pop_clean.csv")) |> 
  filter(sex=="Female")
d_female <- d_female |> 
  pivot_longer(x0:x100) |> 
  mutate(age = as.numeric(str_remove(name, "x"))) 

# Construct 5-year age group brackets to match life tables and fertility data
...
```{r}
# unique(df$Location) 
# excluding as it prints a long list in PDF
...
```{r}
# Calculate 2020 discrete Net Reproduction Rate (NRR) for Australia and Zimbabwe
df |> 
...
Note 2: From the WPP data we have a split first age group, but want to convert this into 0-5. Just sum the Lx values to get $_5L_0$

```{r}
# Function to construct a Leslie Matrix using survivorship and fertility profiles
leslie <- function(nLx,
                   nFx, 
...
```{r}
# Prepare Australia's 2020 demographic data and construct its Leslie matrix
d_aus <- dl |> 
...
# Prepare Zimbabwe's 2020 demographic data and construct its Leslie matrix
d_zim <- dl |> 
...
```{r}
# Verify NRR values calculated from the Leslie matrices
...
```{r}
# Aggregate and plot the 2020 female age distributions (half-pyramid)
dpop <- d_female |> 
...
# Project the female populations forward 200 years (40 steps of 5-year intervals)
n <- 5
age_groups <- seq(0, 100, by = n)
n_age_groups <-  length(age_groups)
n_projections <- 200/n

# Australia projection loop
...
```{r, eval=T}
# Convert projection matrices back to long format and compute age-group proportions
Kdf <- as_tibble(K, .name_repair = "minimal")
...
```{r}
# Plot total projected population size over time
Kdf_long |> 
  group_by(year) |> 
...
```{r}
# Plot the proportion of the population in each age group over time
Kdf_long |> 
